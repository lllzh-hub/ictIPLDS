...
@register()
class DetCriterion(torch.nn.Cell):
    ...
    def _get_positive_nums(self, indices):
        ...
        num_pos = torch.as_tensor([num_pos], dtype=mindspore.float32, device=indices[0][0].device)
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes):
        ...
        src_boxes = _box_convert(src_boxes, in_fmt=self.box_fmt, out_fmt='xyxy')
        target_boxes = _box_convert(target_boxes, in_fmt=self.box_fmt, out_fmt='xyxy')
        iou, _ = box_ops.elementwise_box_iou(src_boxes, target_boxes)
        ...
        src_score = F.sigmoid(src_logits)
