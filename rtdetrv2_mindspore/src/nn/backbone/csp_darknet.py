...
class CSPDarkNet(nn.Cell):
    ...
    def __init__(self, in_channels=3, width_multi=1.0, depth_multi=1.0, return_idx=[2, 3, -1], act='silu', ) -> None:
        ...
        self.layers = nn.CellList([Conv(in_channels, channels[0], 6, 2, 2, act=act)])
...
class CSPPAN(nn.Cell):
    ...
    def __init__(self, in_channels=[256, 512, 1024], depth_multi=1., act='silu') -> None:
        ...
        self.fpn_stems = nn.CellList([Conv(cin, cout, 1, 1, act=act) for cin, cout in zip(in_channels[::-1], in_channels[::-1][1:])])
        self.fpn_csps = nn.CellList([C3(cin, cout, depth, False, act=act) for cin, cout in zip(in_channels[::-1], in_channels[::-1][1:])])

        self.pan_stems = nn.CellList([Conv(c, c, 3, 2, act=act) for c in in_channels[:-1]])
        self.pan_csps = nn.CellList([C3(c, c, depth, False, act=act) for c in in_channels[1:]])
