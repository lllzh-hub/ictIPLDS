...
class YAMLConfig(BaseConfig):
    ...
    def evaluator(self):
        if self._evaluator is None and 'evaluator' in self.yaml_cfg:
            if self.yaml_cfg['evaluator']['type'] == 'CocoEvaluator':
                from . import get_coco_api_from_dataset
    ...
    @staticmethod
    def get_optim_params(cfg: dict, model):
        assert 'type' in cfg, ''
        ...
        for pg in cfg['params']:
            pattern = pg['params']
            params = {k: v for k, v in model.parameters_and_names()
                      if v.requires_grad and len(re.findall(pattern, k)) > 0}
            pg['params'] = params.values()
            param_groups.append(pg)
            visited.extend(list(params.keys()))
        names = [k for k, v in model.parameters_and_names() if v.requires_grad]
    ...
    def get_rank_batch_size(cfg):
        assert ('total_batch_size' in cfg or 'batch_size' in cfg)             and not ('total_batch_size' in cfg and 'batch_size' in cfg)
