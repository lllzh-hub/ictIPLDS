...
class BaseSolver(object):
    ...
    def _setup(self, ):
        ...
        self.model = dist_utils.warp_model(self.model, sync_bn=cfg.sync_bn, \
    ...
    def load_resume_state(self, path: str):
        """load resume
        """
        # NOTE: 不使用 torch.hub，仅支持本地路径加载（MSAdapter/Ascend 兼容）
        if path.startswith('http'):
            raise ValueError(
                f'不支持 http 远程加载权重（MSAdapter/Ascend 不支持 torch.hub）。'
                f'\n请手动下载到本地后再指定本地路径：{path}'
            )
