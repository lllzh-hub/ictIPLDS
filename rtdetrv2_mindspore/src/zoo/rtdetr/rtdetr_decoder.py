...
class MSDeformableAttention(nn.Cell):
    ...
    def _reset_parameters(self):
        ...
        thetas = torch.arange(self.num_heads, dtype=mindspore.float32) * (2.0 * math.pi / self.num_heads)
        grid_init = torch.stack([thetas.cos(), thetas.sin()], -1)
        grid_init = grid_init / grid_init.abs().max(-1, keepdims=True).values
        grid_init = grid_init.reshape(self.num_heads, 1, 1, 2).tile([1, self.num_levels, self.num_points, 1])
        scaling = torch.arange(1, self.num_points + 1, dtype=mindspore.float32).reshape(1, 1, -1, 1)
        grid_init *= scaling
        self.sampling_offsets.bias.set_data(self.sampling_offsets.bias.data)
...
@register()
class RTDETRTransformer(nn.Cell):
    ...
    def _generate_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            valid_WH = mindspore.Tensor([w, h]).to(dtype)
    ...
    def _get_decoder_input(self,
        ...
        _, topk_ind = torch.topk(enc_outputs_class.max(-1)[0], self.num_queries, dim=1)
