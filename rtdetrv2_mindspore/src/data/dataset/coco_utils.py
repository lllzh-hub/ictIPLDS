...
# # import mindspore.mint.utils  # not available.data  # not available
# import mindspore.mint as torch
...
def convert_coco_poly_to_mask(segmentations, height, width):
    if not _FASTER_COCO_EVAL_AVAILABLE:
        raise ModuleNotFoundError(
            ...
            "Please `pip install faster_coco_eval` to use COCO utilities."
        )
    masks = []
...
class ConvertCocoPolysToMask:
    def __call__(self, image, target):
        ...
        iscrowd = mindspore.Tensor([obj["iscrowd"] for obj in anno])
        target["area"] = area
