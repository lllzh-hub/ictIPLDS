...
# import mindspore.mint.distributed  # not available as tdist
...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        if not is_dist_available_and_initialized():
            return
        t = mindspore.Tensor([self.count, self.total], dtype=mindspore.float64, device='cuda')
        tdist.barrier()
        tdist.all_reduce(t)
        t = t.tolist()
        self.count = int(t[0])
...
def all_gather(data):
    ...
    local_size = mindspore.Tensor([tensor.numel()], device="cuda")
    size_list = [mindspore.Tensor([0], device="cuda") for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.empty((max_size,), dtype=mindspore.uint8, device="cuda"))
    if local_size != max_size:
        padding = torch.empty(size=(max_size - local_size,), dtype=mindspore.uint8, device="cuda")
